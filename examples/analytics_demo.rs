...
async fn main() -> Result<(), WarpError> {
    ...
    for i in 0..10 {
        ...
        metadata.insert(
            "action".to_string(),
            serde_json::Value::String(format!("theme_change_{}", i)),
        );
        ...
        .track_event(AnalyticsEvent {
            ...
            metadata: {
                ...
                map.insert(
                    ...
                    serde_json::Value::Number(serde_json::Number::from(340)),
                );
                map
            },
            performance_data: Some(PerformanceData {
    ...
    analytics
        .track_event(AnalyticsEvent {
            ...
            metadata: {
                ...
                map.insert(
                    "error".to_string(),
                    serde_json::Value::String("API timeout".to_string()),
                );
                map.insert(
                    "stack_trace".to_string(),
                    serde_json::Value::String("at ai_request:42".to_string()),
                );
                map
            },
            performance_data: None,
    ...
    analytics
        .track_event(AnalyticsEvent {
            ...
            metadata: {
                let mut map = HashMap::new();
                map.insert(
                    "crash_info".to_string(),
                    serde_json::Value::String("Segmentation fault".to_string()),
                );
                map
            },
            performance_data: None,
    ...
    match analytics
        .get_usage_metrics("catppuccin-theme", TimeRange::LastDay)
        .await
    {
        Ok(metrics) => {
            ...
            println!(
                ...
                metrics.total_usage_time.num_minutes()
            );
            println!(
                "      • Average Session Duration: {} minutes",
                metrics.average_session_duration.num_minutes()
            );
            println!("      • Error Rate: {:.2}%", metrics.error_rate * 100.0);
            println!(
                "      • Performance Score: {:.1}/10",
                metrics.performance_score * 10.0
            );
        }
    ...
    match analytics
        .get_performance_metrics("git-enhanced", TimeRange::LastDay)
        .await
    {
        Ok(metrics) => {
            ...
            println!(
                ...
                metrics.average_load_time.num_milliseconds()
            );
            println!(
                "      • Average CPU Usage: {:.1}%",
            ...
            println!(
                ...
                metrics.average_memory_usage as f64 / (1024.0 * 1024.0)
            );
            println!(
            ...
            println!(
                ...
                metrics.resource_efficiency * 100.0
            );
            println!(
                "      • Stability Score: {:.1}/1.0",
    ...
    match analytics
        .get_marketplace_analytics(TimeRange::LastWeek)
        .await
    {
        Ok(marketplace) => {
            ...
            println!(
                "      • Total Revenue: ${:.2}",
                marketplace.revenue_metrics.total_revenue
            );
            println!(
                "      • Conversion Rate: {:.1}%",
                marketplace.revenue_metrics.conversion_rate * 100.0
            );
            println!(
                "      • Trending Items: {}",
                marketplace.trending_items.len()
            );
        }
